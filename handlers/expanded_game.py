...
async def word_game(callback: types.CallbackQuery):
    await bot.delete_message(callback.from_user.id, callback.message.message_id)
    item1 = types.InlineKeyboardButton('Да, поехали', callback_data='first_word_user_words_game')
...
async def main_words_game(message: types.Message, state: FSMContext):
    ...
    if message.text[0].upper() != last_letter_first.upper():
        await bot.send_message(message.chat.id, f'Так не пойдет, твое слово должно начинатся на: '
                                                f'<b><u>{last_letter_first}</u></b>', parse_mode='html')
        await words_game.first_word_bot.set()
        return

    with open(f'./Game/Words_to_words_game/{message.text[-1].upper()}.txt', encoding='utf-8') as inp:
        letter = inp.readlines()
    i = random.randint(1, len(letter))
    last_letter = letter[i].split('\t')
    last_letter[0] = last_letter[0][-1]
